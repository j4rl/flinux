...
function renderLauncherApps(query=search.value){
  ...
  const grid=document.querySelector('#app-grid');grid.innerHTML='';entries.filter(([id,a])=>`${a.name} ${id} ${a.package||''}`.toLowerCase().includes(query.toLowerCase())).forEach(([id,a])=>{const b=document.createElement('button');b.className='app-tile';b.dataset.app=id;b.innerHTML=`<span class="app-icon category-${a.category}">${a.icon}</span><span>${a.name.replace(' · Textredigerare','').replace(' · Paket','')}</span>`;b.onclick=()=>openApp(id);grid.append(b);});if(!grid.children.length)grid.innerHTML='<p class="empty-apps">Inga program här än.<br>Hitta något nytt i Discover.</p>';
}
