...
export function createShellSession({ system, PACKAGE_REGISTRY = DEFAULT_REGISTRY, openApp = () => {}, onClear = () => {}, onExit = () => {}, onRead = null, cwd = system.state.user.home } = {}) {
  ...
  const commands = {
    ...
    ls: args => {
      ...
      if (node.type === 'dir' && (!training.can(target, currentUser, 'read') || !training.can(target, currentUser, 'execute'))) throw new Error(`ls: ${target}: Permission denied`);
    ...
    ping: args => { ensureArgs(args, 1, 'ping VÄRD'); const host = args.find(a => !a.startsWith('-')); const resolved = training.resolveHost(host); if (!resolved) return result('', 2, line(`ping: ${host}: Temporary failure in name resolution`)); return line(`PING ${host} (${resolved}) 56(84) bytes of data.\n64 bytes from ${resolved}: icmp_seq=1 ttl=64 time=0.42 ms\n64 bytes from ${resolved}: icmp_seq=2 ttl=64 time=0.39 ms\n\n--- ${host} ping statistics ---\n2 packets transmitted, 2 received, 0% packet loss`); },
