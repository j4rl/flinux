...
export function createShellSession({ system, PACKAGE_REGISTRY = DEFAULT_REGISTRY, openApp = () => {}, onClear = () => {}, onExit = () => {}, onRead = null, cwd = system.state.user.home } = {}) {
  let current = system.normalize(cwd), previous = current, lastCode = 0;
  const history = [], environment = Object.assign(Object.create(null), { USER: system.state.user.name, HOME: system.state.user.home, SHELL: '/bin/flinux', PATH: '/usr/bin:/bin', LANG: 'sv_SE.UTF-8', TERM: 'flinux-256color' });
  ...
  const read = value => { const node = system.getNode(path(value)); if (!node) throw new Error(`${value}: Filen eller katalogen finns inte`); if (node.type !== 'file') throw new Error(`${value}: Är en katalog`); return node.content; };
  ...
  const processes = () => [{ pid: 1, user: 'root', command: 'flinux-core' }, { pid: 12, user: environment.USER, command: system.state.settings.desktop + '-desktop' }, { pid: 28, user: environment.USER, command: 'flinux-sh' }, ...Object.keys(PACKAGE_REGISTRY).filter(name => PACKAGE_REGISTRY[name].service && system.state.installed.includes(name) && system.getService(name).active).map((name, i) => ({ pid: 40 + i, user: 'www-data', command: name }))];
  const listProcesses = () => line('  PID USER       COMMAND\n' + processes().map(process => `${String(process.pid).padStart(5)} ${process.user.padEnd(10)} ${process.command}`).join('\n'));
  const walk = (start, visit) => { const normalized = path(start), node = system.getNode(normalized); if (!node) throw new Error(`${start}: Sökvägen finns inte`); const recurse = (entry, name) => { visit(entry, name); if (entry.type === 'dir') for (const child of Object.keys(entry.children).sort()) recurse(entry.children[child], name === '/' ? '/' + child : name + '/' + child); }; recurse(node, normalized); };
  const manuals = {
    ...
    chmod: 'chmod 644 FIL... — ändra visade Unix-rättigheter. Rättigheter är metadata i simulatorn; alla filer tillhör dig.',
  ...
  const commands = {
    help: () => line('flinux 1.0 Glimten — Det glada linuxet\n\nFiler:     pwd cd ls cat touch mkdir rm rmdir cp mv find stat chmod\nText:      echo printf head tail wc sort uniq cut tr basename dirname\nSystem:    whoami hostname id uname date uptime ps df du free env export\nPaket:     apt dpkg which systemctl journalctl\nTerminal:  help man history clear exit true false\n\nInstallationsbara verktyg: tree grep sed jq less diff base64 cal figlet\n  neofetch cowsay fortune htop apachectl curl wget archive\n\nProva: printf "hej\\nflinux\\nhej\\n" | sort | uniq -c\n  echo "Det glada linuxet" > hej.txt && cat hej.txt\n  sudo apt install fortune cowsay && fortune | cowsay\n\nSkriv man shell för syntax. Tab kompletterar, ↑/↓ visar historik.'),
    ...
    pwd: () => line(current),
    cd: args => { const target = args[0] === '-' ? previous : path(args[0] || environment.HOME), node = system.getNode(target); if (node?.type !== 'dir') throw new Error(`cd: ${args[0] || target}: Katalogen finns inte`); previous = current; current = target; return args[0] === '-' ? line(current) : ''; },
    ls: args => {
      ...
      return line(names.map(name => { const child = node.type === 'dir' ? node.children[name] : node, size = child.type === 'file' ? bytes(child.content) : Object.keys(child.children).length; return `${permissions(child)} ${environment.USER} ${String(f.includes('h') ? humanSize(size) : size).padStart(9)} ${new Date(child.modified || system.startedAt).toLocaleDateString('sv-SE')} ${name}`; }).join('\n'));
    },
    ...
    touch: args => { const files = fileArgs(args); ensureArgs(files, 1, 'touch FIL...'); for (const value of files) { const node = system.getNode(path(value)); if (node?.type === 'dir') { node.modified = system.now(); system.save(); } else system.writeFile(path(value), node?.content || ''); } return ''; },
    mkdir: args => { const paths = fileArgs(args); ensureArgs(paths, 1, 'mkdir [-p] KATALOG...'); for (const value of paths) { if (flags(args).includes('p')) { let built = ''; for (const part of path(value).split('/').filter(Boolean)) { built += '/' + part; const node = system.getNode(built); if (!node) system.mkdir(built); else if (node.type !== 'dir') throw new Error(`mkdir: ${built}: Är inte en katalog`); } } else system.mkdir(path(value)); } return ''; },
    rm: args => { const files = fileArgs(args), f = flags(args); ensureArgs(files, 1, 'rm [-rf] FIL...'); for (const value of files) { const node = system.getNode(path(value)); if (!node && f.includes('f')) continue; if (node?.type === 'dir' && !/[rR]/.test(f)) throw new Error(`rm: ${value}: Är en katalog; använd -r`); system.remove(path(value)); } return ''; },
    rmdir: args => { ensureArgs(args, 1, 'rmdir KATALOG...'); for (const value of args) { const node = system.getNode(path(value)); if (node?.type !== 'dir') throw new Error(`rmdir: ${value}: Är inte en katalog`); if (Object.keys(node.children).length) throw new Error(`rmdir: ${value}: Katalogen är inte tom`); system.remove(path(value)); } return ''; },
    cp: args => transfer(args, false), mv: args => transfer(args, true),
    chmod: args => { ensureArgs(args, 2, 'chmod 644 FIL...'); args.slice(1).forEach(value => system.chmod(path(value), args[0])); return ''; },
    ...
    whoami: () => line(system.state.user.name), hostname: () => line(system.state.user.host),
    id: () => line(`uid=1000(${environment.USER}) gid=1000(${environment.USER}) groups=1000(${environment.USER}),27(sudo)`),
    ...
    sudo: (args, stdin, context) => { ensureArgs(args, 1, 'sudo KOMMANDO'); if (args[0] === 'sudo') throw new Error('sudo: nästlad sudo behövs inte i flinux'); return run(args, stdin, context); },
    ...
    tree: args => { const output = [path(fileArgs(args)[0] || '.')], root = system.getNode(output[0]); if (!root) throw new Error('tree: sökvägen finns inte'); const visit = (node, prefix) => { if (node.type !== 'dir') return; const entries = Object.entries(node.children).filter(([name]) => flags(args).includes('a') || !name.startsWith('.')).sort(([a], [b]) => a.localeCompare(b)); entries.forEach(([name, child], i) => { const last = i === entries.length - 1; output.push(prefix + (last ? '└── ' : '├── ') + name); visit(child, prefix + (last ? '    ' : '│   ')); }); }; visit(root, ''); return line(output.join('\n')); },
  ...
  function transfer(args, move) {
    ...
    for (const source of files) { if (!move && system.getNode(path(source))?.type === 'dir' && !/[rR]/.test(flags(args))) throw new Error('cp: kataloger kräver -r'); system[move ? 'move' : 'copy'](path(source), path(destination)); } return '';
  }
  ...
  function archive(args) {
    ...
    if (action === 'create') {
      ...
      for (const value of entries) { const initial = path(value), base = initial.split('/').pop(); if (!base) throw new Error('archive: välj filer eller underkataloger'); walk(initial, (node, absolute) => { const name = base + absolute.slice(initial.length); if (seen.has(name)) throw new Error('archive: filnamn kolliderar'); seen.add(name); stored.push({ path: name, type: node.type, mode: node.mode, ...(node.type === 'file' ? { content: node.content } : {}) }); }); }
      system.writeFile(path(archivePath), JSON.stringify({ format: 'flinux-archive-1', entries: stored }, null, 2)); return line(`Skapade ${archivePath}: ${stored.length} poster.`);
    }
    ...
    for (const entry of archive.entries) {
      if (typeof entry.path !== 'string' || entry.path.startsWith('/') || entry.path.split('/').some(part => !part || part === '..' || part === '.') || entry.path.includes('\0') || !['file', 'dir'].includes(entry.type) || (entry.type === 'file' && typeof entry.content !== 'string')) throw new Error('archive: osäker eller ogiltig arkivpost');
      const target = system.normalize(destination + '/' + entry.path); if (!target.startsWith(destination === '/' ? '/' : destination + '/') || targets.has(target) || system.getNode(target)) throw new Error('archive: målfilen finns redan eller sökvägen är ogiltig'); targets.set(target, entry);
    }
    const ordered = [...targets.entries()].sort(([a], [b]) => a.split('/').length - b.split('/').length);
    for (const [target] of ordered) { const parent = target.slice(0, target.lastIndexOf('/')) || '/'; if (system.getNode(parent)?.type !== 'dir' && targets.get(parent)?.type !== 'dir') throw new Error('archive: överordnad katalog saknas'); }
    for (const [target, entry] of ordered) { if (entry.type === 'dir') system.mkdir(target); else system.writeFile(target, entry.content); if (/^[0-7]{3}$/.test(entry.mode)) system.chmod(target, entry.mode); }
  ...
  return {
    ...
    get prompt() { return `${system.state.user.name}@${system.state.user.host}:${current === environment.HOME || current.startsWith(environment.HOME + '/') ? '~' + current.slice(environment.HOME.length) : current}$`; },
    execute(raw) {
      ...
      for (const group of groups) {
        ...
        for (let i = 0; i < group.commands.length; i++) {
          ...
          try {
            ...
            for (const redirect of redirects) { if (!redirect.path) throw new Error('Omdirigeringen behöver ett filnamn'); if (redirect.operator === '<') stdin = read(redirect.path); else system.writeFile(path(redirect.path), '', redirect.operator === '>>'); }
            ...
            const writes = redirects.filter(redirect => redirect.operator !== '<'); if (writes.length) { const redirect = writes.at(-1); system.writeFile(path(redirect.path), stdin, true); stdin = ''; }
